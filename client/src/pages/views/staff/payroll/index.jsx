...
const PayrollView = () => {
    ...
    const filteredPayrolls = yearFilter
        ? payrolls.filter(
            payroll => new Date(payroll.periodEnd).getFullYear().toString() === yearFilter
        )
        : payrolls;
    ...
    return (
        <div className="relative isolate min-h-lvh overflow-hidden bg-[linear-gradient(to_bottom,#1b1b1b_25%,#FAFAFA_25%)]">
            ...
            <div className="container mx-auto p-4">
                <h1 className="text-3xl text-white font-semibold mb-6">My Payroll History</h1>
                                    ...
                                    filteredPayrolls.map((payroll) => (
                                        <tr key={payroll._id}>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div>{format(new Date(payroll.periodStart), 'MMM d')} - {format(new Date(payroll.periodEnd), 'MMM d, yyyy')}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
